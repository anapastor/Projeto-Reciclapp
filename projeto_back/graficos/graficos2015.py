...
arquivo = r'C:\Users\kalle\Desktop\Importante\Programação\Trabalho 4 º Semstre\Projeto-Reciclapp\projeto_back\graficos\amlurbcoletatiposresiduos2015.csv'
...
# Renomear a coluna com erro de codificação
data.rename(columns={'TIPO DE RESADUO': 'TIPO DE RESIDUO'}, inplace=True)

# Converter colunas de meses para valores numéricos
meses = data.columns[1:-1]  # Selecionar todas as colunas de meses, ignorando a primeira e a última
for mes in meses:
    data[mes] = pd.to_numeric(data[mes], errors='coerce')  # Converter para números, substituindo erros por NaN

# Substituir valores NaN por 0 (opcional, se fizer sentido no contexto)
